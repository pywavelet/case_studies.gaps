...
from ..utils.io import read_inference_data_posterior
...
def _process_input(chains: List[np.ndarray] = None, idata_fnames: List[str] = None, paramNames=None):
    if chains is not None and idata_fnames is not None:
        raise ValueError("Either chains or idata_fnames must be provided, not both.")

    if chains is None and idata_fnames is None:
        raise ValueError("Either chains or idata_fnames must be provided.")

    if idata_fnames:
        chains, paramNames = [], []
        for fname in idata_fnames:
            paramNames, posterior = read_inference_data_posterior(fname)
            chains.append(posterior)
    return chains, paramNames


def plot_corner(
        chains: List[np.ndarray] = None,
        idata_fnames: List[str] = None,
        weights=None,
        chainLabels=None,
        paramNames=None,
        truths=None,
        truthLabels=None,
        truthColors=truthsDefaultColors,
        truthLineStyles=truthsDefaultLS,
        priors=None,
        plotName=None,
        nContourLevels=2,
    ...
    idata_fnames : list of strings
        List of InferenceData file names to load chains from.
        Note -- either chains or idata_fnames must be provided, not both.
    ...
    # Process input
    chains, paramNames = _process_input(chains, idata_fnames, paramNames)
